...
DUMP_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'database', 'database_dump.sql')
    ...
    with app.app_context():
        ...
        if 'doctors' in data:
            ...
            for row in data['doctors']:
                d = Doctor(id=row[0], name=row[1], specialization=row[2], email=row[3])
        ...
        if 'users' in data:
            ...
            for row in data['users']:
                ...
                u = User(
                    ...
                    gender=row[6], blood_group=row[7], govt_id=row[8], health_id=row[9],
                    address=row[10], city=row[11], state=row[12], pincode=row[13],
        ...
        if 'medical_records' in data:
            ...
            for row in data['medical_records']:
                ...
                mr = MedicalRecord(
                    ...
                    doctor_name=row[3], facility=row[4], type=row[5],
                    title=row[6], diagnosis=row[7], file_url=row[8],
                    created_at=created_at_dt
                )
        ...
        if 'prescriptions' in data:
            ...
            for row in data['prescriptions']:
                ...
                rx = Prescription(
                    id=row[0], patient_id=row[1], rx_code=row[2],
                    doctor_name=row[3], facility=row[4],
                    ...
                    diagnosis=row[7], status=row[8],
                    medicines=json.loads(row[9]) if row[9] else None,
                    pharmacy_note=row[10], transaction_id=row[11],
                    created_at=created_at_dt,
