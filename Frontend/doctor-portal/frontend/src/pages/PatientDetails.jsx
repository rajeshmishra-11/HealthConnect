...
const PatientDetails = () => {
    ...
    useEffect(() => {
        const fetchPatientData = async () => {
            try {
                const data = mockDb.getPatientById(id);
                if (data) {
                    setPatient(data);
                    setVisits(mockDb.getVisitsByPatient(id));
                    setPrescriptions(mockDb.getPrescriptionsByPatient(id));
                }
                setLoading(false);
            } catch (error) {
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-card p-8 rounded-3xl border border-border shadow-sm flex flex-col lg:flex-row justify-between gap-8">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <span className="px-3 py-1 bg-secondary rounded-full text-xs font-bold uppercase tracking-wider text-muted-foreground">{patient.gender} • {patient.age} Yrs</span>
                        </div>
                        <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm">
                            ...
                            <span className="flex items-center gap-2 text-muted-foreground">
                                ...
                                <span>Blood Group: <span className="font-bold text-foreground">{patient.bloodGroup}</span></span>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <Link
                        to={`/visits/create?patientId=${patient.id}`}
                    ...
                    <Link
                        to={`/prescriptions/create?patientId=${patient.id}`}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    ...
                                    <div key={visit.id} className="bg-card p-6 rounded-2xl border border-border hover:border-primary/30 transition-all group">
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                ...
                                                <h3 className="text-xl font-bold text-foreground">{visit.diagnosis}</h3>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                            <div>
                                                <span className="font-bold text-muted-foreground uppercase text-[10px] tracking-widest block mb-1">Symptoms</span>
                                                <p className="text-foreground">{visit.symptoms}</p>
                                            </div>
                                            <div>
                                                <span className="font-bold text-muted-foreground uppercase text-[10px] tracking-widest block mb-1">Doctor's Notes</span>
                                                <p className="text-foreground">{visit.notes || "No notes provided."}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                                ...
                                {prescriptions.map((px) => (
                                    <div key={px.id} className="bg-card p-6 rounded-2xl border border-border group hover:shadow-sm transition-all flex flex-col md:flex-row md:items-center justify-between gap-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="text-sm font-bold text-foreground">{px.rxCode}</div>
                                                <div className="text-xs text-muted-foreground">{px.date}</div>
                                            </div>
                                        </div>
                        ...
                        {activeTab === "info" && (
                            <div className="bg-card p-8 rounded-3xl border border-border divide-y divide-border">
                                ...
                                <div className="py-4 grid grid-cols-3">
                                    ...
                                    <span className="text-sm font-semibold col-span-2 leading-relaxed">{patient.address}</span>
                                </div>
                                <div className="py-4 grid grid-cols-3">
                                    <span className="text-sm font-bold text-muted-foreground">Vitals Profile</span>
                                    <div className="col-span-2 flex gap-6">
                                        <div>
                                            <div className="text-[10px] uppercase font-bold text-muted-foreground mb-1">Height</div>
                                            <div className="text-sm font-semibold">{patient.height}</div>
                                        </div>
                                        <div>
                                            <div className="text-[10px] uppercase font-bold text-muted-foreground mb-1">Weight</div>
                                            <div className="text-sm font-semibold">{patient.weight}</div>
                                        </div>
                                    </div>
                                </div>
                                <div className="py-4 last:pb-0 grid grid-cols-3">
                                    <span className="text-sm font-bold text-muted-foreground">Emergency</span>
                                    <span className="text-sm font-semibold col-span-2 text-red-500">{patient.emergencyContact}</span>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="space-y-6">
                    <div className="bg-primary p-6 rounded-3xl text-white shadow-lg shadow-primary/20">
                        ...
                        <p className="text-[10px] text-white/50 mt-4 text-center italic">Measured on Feb 15, 2024</p>
                    </div>

                    <div className="bg-card p-6 rounded-3xl border border-border">
                        <h3 className="font-bold text-foreground mb-4">Known Allergies</h3>
                        <div className="flex flex-wrap gap-2">
                            <span className="px-3 py-1 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 rounded-lg text-xs font-bold">Penicillin</span>
                            <span className="px-3 py-1 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 rounded-lg text-xs font-bold">Peanuts</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-muted/50 p-6 rounded-3xl border border-border border-dashed">
                        ...
                        <p className="text-xs text-muted-foreground leading-relaxed">
                            Patient exhibits mild hypertension. Advised reduction in sodium intake and regular light exercise.
                        </p>
                    </div>
                </div>
            </div>
